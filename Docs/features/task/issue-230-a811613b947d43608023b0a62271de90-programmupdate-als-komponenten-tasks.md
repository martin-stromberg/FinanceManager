...
Stand: Plan-Review vom 2026-07-30 (fuenfter Durchlauf). Umgesetzt: 123 von 126. Offen: 3 (Aufgaben 21, 125 und 126).

Gegenueber dem Vorreview geschlossen: keine (21 und 125 bestehen unveraendert).
Neu erkannt: 126 (Ausnahmen aus Event-Handlern werden nicht als `LastError` im Status-Service abgelegt, obwohl `plan.md` Zeile 22 das ausdruecklich fordert).

Sanity-Check: `dotnet build FinanceManager.sln -c Debug` → 0 Fehler, 60 Warnungen (saemtlich Vorbestand). `dotnet test SoftwareSchmiede.AutoUpdate.Tests` → 103/104 gruen (1 plattformbedingt uebersprungen), `dotnet test FinanceManager.Tests --filter Updates` → 39/39 gruen.
...
| 2 | Projektstruktur | Paketreferenzen der Bibliothek setzen (`Microsoft.Extensions.Hosting.Abstractions`, `Options`, `Logging.Abstractions`, `Http`, `DependencyInjection.Abstractions`, `Configuration.Binder`) | Erledigt | `UseAutoUpdateRegistrationTests.UseAutoUpdate_RegistersAllServices` (alle 6 Referenzen gesetzt) |
...
| 4 | Projektstruktur | Testprojekt `SoftwareSchmiede.AutoUpdate.Tests.csproj` anlegen und in `FinanceManager.sln` eintragen | Erledigt | Kein direkter Test (104 Tests laufen im Projekt) |
...
| 7 | Datenmodell | `AutoUpdateResult` Record anlegen | Erledigt | `AutoUpdateCommandServiceTests.Check_DelegatesToOrchestrator` |
...
| 18 | Konfiguration | `AutoUpdateOptions` als laufzeitveränderliche Singleton-Optionsklasse anlegen | Erledigt | `AutoUpdateBuilderTests.Builder_FluentChain_SetsAllOptions` (alle 13 Planfelder vorhanden, zusaetzlich `UpdateUnitName`) |
...
| 21 | Konfiguration | `UpdateOptions` (Web) um `SourceType`, `LocalFolderPath`, `EnableAutomaticDownload`, `EnableAutomaticInstallation` erweitern | Offen | — (nur `SourceType` und `LocalFolderPath` vorhanden; beide Schalter fehlen weiterhin. Funktional folgenlos, da `Updates:EnableAutomaticDownload`/`...Installation` per `BindConfiguration("Updates")` direkt auf `AutoUpdateOptions` binden — alternativ Plan an den schlankeren Vertrag anpassen) |
| 22 | Konfiguration | Sektion `Updates` in `appsettings.json` um die neuen Einträge ergänzen | Erledigt | Kein direkter Test (alle 7 Planeintraege vorhanden, verhaltensneutral fuer Bestandsinstallationen) |
...
| 28 | Schnittstellen | `IAutoUpdatePackageValidator` mit `IsNewerVersion` und `ValidateDownloadedPackageAsync` definieren (Plan Zeile 166: kein `ValidateReleaseAsync`) | Erledigt | `AutoUpdatePackageValidatorTests.IsNewerVersion_ComparesInstalledAndAvailableVersions`, `ValidateDownloadedPackageAsync_AcceptsMatchingPackage` |
...
| 34 | Schnittstellen | `IAutoUpdateOrchestrator` definieren | Erledigt | `AutoUpdateCommandServiceTests.Check_DelegatesToOrchestrator`, `Download_DelegatesToOrchestrator`, `Install_DelegatesToOrchestrator` |
...
| 42 | Events | Raise-Methoden inkl. Handler-Ausnahmebehandlung (Meldung über `ErrorOccurred`, kein Abbruch) implementieren | Erledigt | `AutoUpdateEventsTests.Raise_WhenHandlerThrows_ReportsErrorAndContinues`, `Raise_AfterStartUpdateScript_HasNoCancelSemantics` (Ablage als `LastError` fehlt — siehe Aufgabe 126) |
...
| 46 | Logik | `FileSystemAutoUpdateStateStore` inkl. tolerantem Fallback bei fremdem Schema implementieren | Erledigt | `AutoUpdateStatusServiceTests.Load_WithUnreadableStateFile_FallsBackToIdle` (inkl. Warnung im Log, Offener Punkt 7) |
...
| 55 | Logik | `AutoUpdateInstaller` (PrepareAsync, StartAsync) aus `UpdateExecutor` portieren | Erledigt | `AutoUpdateOrchestratorInstallTests.Install_GeneratesScriptAndStartsIt` (Startmethode heisst `Start(string)` und ist synchron — Namensabweichung ohne inhaltliche Luecke) |
...
| 59 | Logik | `AutoUpdateGithubSource` inkl. statischer Factory implementieren | Erledigt | `AutoUpdateGithubSourceTests.Create_WithEmptyOwner_Throws`, `Check_ParsesManifestResponse`, `Download_WhenHttpFails_Throws` (Signatur `Create(repositoryOwner, repositoryName, manifestAssetName?)` — Parameterreihenfolge abweichend vom Plantext) |
...
| 65 | Logik | Zentrale Fehlerbehandlung des Orchestrators (Ausnahme → `ErrorOccurred` + Zustand `Failed` + `Outcome.Failed`) implementieren | Erledigt | `AutoUpdateOrchestratorCheckTests.Check_WhenSourceThrows_ReportsErrorAndFails` |
...
| 67 | Logik | `AutoUpdateCommandService` als Fassade über den Orchestrator implementieren | Erledigt | `AutoUpdateCommandServiceTests.Check_DelegatesToOrchestrator`, `Commands_UpdateStatusService` |
| 68 | Hintergrunddienste | `AutoUpdateCheckerService` mit Intervall- und Zeitfensterprüfung implementieren | Erledigt | `AutoUpdateCheckerServiceTests.Execute_RunsUpdateWorkflow_WithinWindow`, `Execute_DoesNotRun_OutsideWindow`, `Execute_OnlyCallsRunUpdateAsync_NeverIndividualSteps`, `Execute_WhenRunThrows_ContinuesLoop`, `Execute_RespectsConfiguredInterval` (Dienst ruft `RunUpdateAsync` gemaess Programmablauf, nicht `CheckForUpdateAsync` gemaess Umsetzungsreihenfolge — Planwiderspruch, siehe `review.md`) |
...
| 76 | Integration | `UpdateSettingsStore` auf `IAutoUpdatePackageStore` und `AutoUpdateOptions` umstellen, `ApplyToOptionsAsync` ergänzen | Erledigt | `UpdateSettingsStoreTests.ApplyToOptions_TransfersSettingsIntoAutoUpdateOptions`, `UpdateOrchestratorAdapterTests.Adapter_SaveSettings_AppliesToAutoUpdateOptions` (Methode heisst `ApplyToOptions` und ist synchron; `IOptions<UpdateOptions>` bleibt zusaetzlich injiziert — Abweichungen ohne inhaltliche Luecke) |
...
| 83 | Aufräumen | `UpdateContracts.cs` auf `IUpdateSettingsStore`, `IInstalledReleaseMetadataProvider`, `IUpdateOrchestrator` reduzieren | Erledigt | Kein direkter Test (Datei enthaelt genau diese 3 Interfaces) |
...
| 87 | Tests | `UseAutoUpdateRegistrationTests` schreiben | Erledigt | u. a. `UseAutoUpdate_DoesNotOverrideExistingTimeProvider` |
...
| 96 | Tests | `AutoUpdateCommandServiceTests` schreiben | Erledigt | 5 Tests (`Commands_DelegateToOrchestrator` in drei Einzeltests aufgeteilt) |
...
| 100 | Tests | `AutoUpdateCheckerServiceTests` schreiben | Erledigt | 5 Tests (Namen an `RunUpdateAsync` angepasst) |
...
| 103 | Tests | `AutoUpdateScriptGeneratorTests` schreiben | Erledigt | 3 Tests (Linux-Test plattformbedingt uebersprungen) |
...
| 108 | Tests | `UpdateOrchestratorAdapterTests` in `FinanceManager.Tests/Updates/` schreiben | Erledigt | 3 Tests (+ `UpdateOrchestratorAdapterLockAndScheduleTests`) |
| 109 | Tests | Verschobene Testklassen aus `FinanceManager.Tests/Updates/` entfernen | Erledigt | Kein direkter Test (Build 0 Fehler, keine Referenz auf entfallene Web-Typen) |
| 110 | Tests | `UpdateSettingsStoreTests` an die neuen Abhängigkeiten anpassen | Erledigt | `UpdateSettingsStoreTests` (39 Update-Tests gruen) |
...
| 113 | Tests | `UpdateControllerIntegrationTests` auf lokale Quelle und deaktivierte Hintergrunddienste umstellen | Erledigt | `UpdateControllerIntegrationTests` |
| 114 | E2E-Tests | `PlaywrightWebAppFixture` um `Updates__*`-Umgebungsvariablen erweitern | Erledigt | `UpdateSetupPlaywrightTests.Admin_TriggersCheck_ShowsAvailableUpdate` (alle 5 Planvariablen gesetzt) |
...
| 120 | Dokumentation | `README.md` der Bibliothek (Verwendung, Optionen, Ereignisse, eigene Quellen, Plattformgrenzen) schreiben | Erledigt | Kein direkter Test (`SoftwareSchmiede.AutoUpdate/README.md`, als `PackageReadmeFile` eingebunden) |
| 121 | Dokumentation | `CHANGELOG.md`-Eintrag ergänzen | Erledigt | Kein direkter Test (Eintrag vorhanden) |
| 122 | Dokumentation | Betroffene Abschnitte in `Docs/help` aktualisieren | Erledigt | Kein direkter Test (`ablauf-technisch.md`, `beschreibung.md`, `business-rules.md`, `bereitstellung.md`, `installation.md`) |
| 123 | Paketierung | `dotnet pack` für `SoftwareSchmiede.AutoUpdate` lauffähig machen und gegen einen lokalen Paketordner prüfen | Erledigt | Kein direkter Test (NuGet-Metadaten inkl. `PackageId`, `Version`, `PackageLicenseExpression`, `PackageReadmeFile` gesetzt) |
...
| 125 | Konfiguration | `UseAutoUpdate` muss `IValidateOptions<AutoUpdateOptions>` in der DI registrieren (Plan, Ablauf „Registrierung beim Start", Schritt 4) | Offen | — (`AutoUpdateOptionsValidator` existiert und wird in `BuildOptions` direkt ausgefuehrt, es gibt aber keine DI-Registrierung. Funktional folgenlos, da die Bibliothek `AutoUpdateOptions` als Instanz-Singleton statt ueber das `IOptions`-Muster registriert — alternativ Plan an die eager ausgefuehrte Validierung anpassen) |
| 126 | Events | Ausnahme eines Event-Handlers zusaetzlich als `LastError` im Status-Service ablegen (Plan, Designentscheidung „Fehler in Event-Handlern", Zeile 22) | Offen | — (`AutoUpdateEvents.RaiseCancelable` faengt die Ausnahme und meldet sie ueber `ErrorOccurred`, der Ablauf laeuft weiter und die Abbruchstimme zaehlt nicht; die vom Plan geforderte Ablage als `LastError` fehlt, da kein Abonnent `ErrorOccurred` mit dem Status-Service verbindet) |
