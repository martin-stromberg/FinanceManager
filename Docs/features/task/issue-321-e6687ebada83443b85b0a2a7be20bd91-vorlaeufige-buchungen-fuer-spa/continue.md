...
- [ ] Draft-Beschreibung `Vorl. Buchungen vom {dateText}` in `StatementDraftService.Preliminary.cs` lokalisiert einbauen (Ressource `StatementDraft_Description_Preliminary`)
- [ ] Validierungswarnung für vorläufige Stornierung als klickbaren Link zur Buchungsübersicht darstellen
