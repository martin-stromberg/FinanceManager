...
{
    [Fact]
    public void Finish_SplitsOverflowIntoUnbudgeted_ForExactPostingExpectation()
    {
        ...
        monthResult.UnbudgetedPostings.Sum(p => p.Amount).Should().Be(-5.98m, "the overrun beyond the -10 budget must be unbudgeted");
    }

    [Fact]
    public void Finish_CombinesMultipleBudgetsPerPurpose_AndMarksExcessUnbudgeted()
    {
        var contactId = Guid.NewGuid();
        ...
        monthResult.UnbudgetedPostings.Sum(p => p.Amount).Should().Be(-2m);
    }
    ...
    [Fact]
    public void Finish_IncomeExpectation_OverrunIsAlsoUnbudgeted()
    {
        var contactId = Guid.NewGuid();
        ...
        monthResult.UnbudgetedPostings.Sum(p => p.Amount).Should().Be(200m);
    }
