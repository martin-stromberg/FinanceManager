...
public sealed class PortfolioStructureCardTests : BunitContext
{
    ...
    /// <summary>
    /// The unrealized gain/loss explanation lists every position (not only the top 10) with its own
    /// unrealized gain/loss value, plus a total row, so the listed values always sum to the displayed total.
    /// </summary>
    [Fact]
    public void UnrealizedGainLossExplanation_ListsAllPositionsWithTotal()
    {
        WithInvariantCulture(() =>
        {
            Services.AddLocalization(options => options.ResourcesPath = "Resources");
            ...
            {
                new(Guid.NewGuid(), "Security A", 600m, 0.6m, 100m),
                new(Guid.NewGuid(), "Security B", 400m, 0.4m, -20m)
            };
            var data = BuildData(1000m, positions);

            var cut = Render<PortfolioStructureCard>(builder => builder.Add(c => c.Data, data));

            // The "unrealized gain/loss" info button is the third .kpi-info-btn in the card.
            cut.FindAll(".kpi-info-btn")[2].Click();

            var rows = cut.FindAll(".kpi-explanation-table tbody tr");
            // 2 position rows + 1 total row.
            Assert.Equal(3, rows.Count);
            Assert.Contains("Security A", rows[0].TextContent);
            Assert.Contains("100.00", rows[0].TextContent);
            Assert.Contains("Security B", rows[1].TextContent);
            Assert.Contains("-20.00", rows[1].TextContent);
        });
    }

    /// <summary>
    /// When more than <c>MaxListEntries</c> (200) positions exist, the unrealized gain/loss explanation
    /// caps the rendered rows at 200 and adds an "and N more" row for the remainder.
    /// </summary>
    [Fact]
    public void UnrealizedGainLossExplanation_MoreThanMaxListEntriesPositions_CapsListAndShowsMoreEntriesRow()
    {
        WithInvariantCulture(() =>
        {
            Services.AddLocalization(options => options.ResourcesPath = "Resources");
            ...
            var positions = Enumerable.Range(1, 201)
                .Select(i => new PortfolioTopPosition(Guid.NewGuid(), $"Security {i}", 1m, 1m / 201m, 1m))
                .ToList();
            var data = BuildData(201m, positions);
    ...
    /// <summary>
    /// When exactly <c>MaxListEntries</c> (200) positions exist, no "and N more" row is rendered
    /// in the unrealized gain/loss explanation (off-by-one boundary: the cap is only exceeded, not
    /// merely reached).
    /// </summary>
    [Fact]
    public void UnrealizedGainLossExplanation_ExactlyMaxListEntriesPositions_NoMoreEntriesRow()
    {
        WithInvariantCulture(() =>
        {
            Services.AddLocalization(options => options.ResourcesPath = "Resources");
            ...
            var positions = Enumerable.Range(1, 200)
                .Select(i => new PortfolioTopPosition(Guid.NewGuid(), $"Security {i}", 1m, 1m / 200m, 1m))
                .ToList();
            var data = BuildData(200m, positions);
    ...
    /// <summary>
    /// The top positions table no longer renders an "Unrealized G/L" column: only name, market value
    /// and share are shown, since the unrealized gain/loss breakdown moved to the KPI info panel.
    /// </summary>
    [Fact]
    public void TopPositionsTable_DoesNotRenderUnrealizedGainLossColumn()
    {
        WithInvariantCulture(() =>
        {
            Services.AddLocalization(options => options.ResourcesPath = "Resources");
            ...
            var headers = cut.FindAll(".portfolio-top-positions thead th");
            Assert.Equal(3, headers.Count);

            var cells = cut.FindAll(".portfolio-top-positions tbody td");
